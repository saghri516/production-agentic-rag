...
LLM_FALLBACK_MESSAGE = "I'm sorry, but I couldn't generate a response right now. Please try again."
...
def _safe_invoke(llm, messages, fallback):
    try:
        return _invoke_with_retry(llm, messages)
    except RetryError as exc:
        print(f"Warning: LLM call failed after 3 attempts: {exc.last_attempt.exception()}")
        return fallback
...
def orchestrator(state: AgentState, llm_with_tools):
    ...
    if not state.get("messages"):
        ...
        response = _safe_invoke(
            ...
            fallback=AIMessage(content=LLM_FALLBACK_MESSAGE),
        )
...
def compress_context(state: AgentState, llm):
    ...
    removals = [RemoveMessage(id=m.id) for m in messages[1:] if getattr(m, "id", None)]
    return {"context_summary": new_summary, "messages": removals}


def collect_answer(state: AgentState):
    last_message = state["messages"][-1]
    is_valid = isinstance(last_message, AIMessage) and last_message.content and not getattr(last_message, "tool_calls", None)
...
def aggregate_answers(state: State, llm):
    ...
    synthesis_response = _safe_invoke(
        llm,
