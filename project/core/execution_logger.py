...
def _log_block(title: str, payload: Any, color: str) -> None:
    if not _enabled():
        return
...
def log_chat_end(state: Any) -> None:
    _log_block("FINAL GRAPH STATE", state_preview(state), "blue")


def log_node_start(name: str, state: Any) -> None:
    _log_block(f"NODE START: {name}", state_preview(state), "cyan")


def log_node_end(name: str, update: Any) -> None:
    _log_block(f"NODE OUTPUT: {name}", update_preview(update), "green")
...
def log_route(name: str, decision: Any, state: Any | None = None) -> None:
    ...
    _log_block(f"ROUTE: {name}", payload, "yellow")


def log_tool_start(name: str, args: dict[str, Any]) -> None:
    _log_block(f"TOOL START: {name}", args, "magenta")


def log_tool_end(name: str, output: Any) -> None:
    _log_block(f"TOOL OUTPUT: {name}", {"output": _truncate(output)}, "magenta")


def log_error(scope: str, error: Exception) -> None:
    _log_block(f"ERROR: {scope}", {"type": error.__class__.__name__, "message": str(error)}, "red")
