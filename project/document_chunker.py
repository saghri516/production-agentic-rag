...
class DocumentChunker:
    ...
    @staticmethod
    def __strip_table_of_contents(md_text: str) -> str:
        """Remove Table of Contents sections before chunking.

        TOC pages are dense with dotted leaders (....) and page numbers,
        which pollute embedding similarity for unrelated real-content queries
        (e.g. a TOC entry "Advantages of JavaScript .... 3" can outscore the
        actual "Advantages of JavaScript" section during retrieval).
        """
        pattern = r"(?:^|\n)#+\s*\*{0,2}Table\s+of\s+Contents\*{0,2}\s*(?:\n|$).*?(?=\n#+\s|\Z)"
        return re.sub(pattern, "", md_text, flags=re.DOTALL | re.IGNORECASE)

    @staticmethod
    def __is_toc_like(text: str, threshold: float = 0.15) -> bool:
        """Detect table-of-contents-style content via dotted leader density.

        Catches TOC/index pages that survive the heading-based strip above
        (e.g. TOCs without a literal "Table of Contents" heading, or ones
        split apart by the parent splitter before the strip could remove
        them as one block).
        """
        lines = text.split("\n")
        dot_leader_lines = sum(1 for line in lines if re.search(r"\.{4,}", line))
        return dot_leader_lines / max(len(lines), 1) > threshold
    ...
    def create_chunks_single(self, md_path, source_name=None):
        ...
        with open(doc_path, "r", encoding="utf-8") as f:
            md_text = f.read()
        md_text = self.__strip_table_of_contents(md_text)
        parent_chunks = self.__parent_splitter.split_text(md_text)
        ...
        cleaned_parents = [c for c in cleaned_parents if not self.__is_toc_like(c.page_content)]
